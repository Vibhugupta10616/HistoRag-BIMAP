#!/bin/bash
# SLURM job: embed on work partition (RTX 2080 Ti / RTX 3080, 10-11 GB VRAM).
# Hardcoded for Hypopharynx CONCH — patches already exist, skips unzip.
# Usage: sbatch ~/HistoRag-BIMAP/HPC/embed_job_work.sh
...
ZIP=$(find "$WORK/hancock/$TISSUE_LOWER/zips/" -name "*.zip" 2>/dev/null | head -1)
...
# --- Step 4: Tile + embed (smaller batch for 10-11 GB VRAM) ---
